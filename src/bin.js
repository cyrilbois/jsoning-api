...
function help() {
    console.log(`Usage: JSONingAPI [options]

    Options:
            -p,  --port <port>        Port (default: 3000)
            -d, --database <file>    Database file (default: db.json)
            -r, --rule <file>        Rule file (default: rules.json)
...
function args() {
    try {
        const options = {
            ...
            rule: { type: 'string', short: 'r', default: process.env['RULE'] ?? path.join(__dirname, '..', 'rules.json') },
            help: { type: 'boolean' }
        };
...
const engine = Engine.createFromFile(rulePath);

const service = new Service(db);
const controller = new Controller(service);

const app = createApp(controller, engine);
